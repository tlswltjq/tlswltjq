...
#introduction([
  ...
  - 또한 트랜스코딩 작업을 이벤트 기반 파이프라인 워커로 분리하여 비즈니스 로직의 복잡도를 줄이고 안정적인 처리를 구현했습니다.
...
#projects(
  ...
  project_block(
    ...
    [
      = 개요
      사용자 간 또는 그룹 간 음성 메시지를 포함한 푸시 알림을, 사용자가 지정한 스케줄에 따라 자동 발송하는 서비스입니다.

      == 사용자 지정 스케줄에 따라 자동 발송되는 푸시 알림
      - 사용자가 입력한 제목, 본문, 음성 파일을 FCM(Firebase Cloud Messaging)을 통해 전달했습니다.
      - 발송 주기는 CRON 표현식 형태로 테이블에 저장되며, 활성화된 알림은 Quartz 스케줄러에 의해 관리됩니다.
      - 스케줄러에 등록된 푸시 알림(Job)이 실행될 때, 발송 대상을 조회해 실제 푸시 알림이 전송되도록 구현했습니다.

      == AWS S3
      - 사용자가 업로드한 음성 파일을 보관하고 재전송하기 위해 AWS S3 버킷을 생성해 저장했습니다.
      - 파일은 푸시 알림과 함께 전송되어, 수신자가 앱에서 음성을 바로 재생할 수 있도록 했습니다.

      == 깃허브 액션과 코드디플로이를 통한 배포
      - GitHub Actions와 AWS CodeDeploy를 이용해 CI/CD 파이프라인을 구축했습니다.
      - 코드 푸시 시 자동으로 빌드, 테스트, 배포가 수행되도록 설정해 배포 과정의 효율성을 높였습니다.
      - 이를 통해 자동화된 배포 흐름과 DevOps 환경의 기본 개념을 익혔습니다.

      == 모바일 앱과 연동
      - Flutter를 사용해 푸시 알림을 수신할 수 있는 모바일 앱을 개발했습니다.
      - 백그라운드 상태에서도 알림이 도착하면 함께 전달된 음성 파일이 자동으로 재생되도록 구현했습니다.

      == 수신자 목록 처리 및 Job 관리 최적화
      - 그룹 단위 푸시 알림 발송 시, 각 그룹원을 대상으로 개별 Job을 스케줄러에 등록하도록 했습니다. \ 하지만 그룹원 변경 시 모든 관련 Job을 수정해야 하는 비효율적인 문제가 발생했습니다. \ 이를 해결하기 위해 발송 유형(개인 / 그룹)을 구분해 Job을 등록하고, 실제 발송 시점에 수신자를 조회하도록 구조를 변경했습니다.
      - 결과적으로 스케줄러 관리가 단순해지고, 그룹원 변동에도 유연하게 대응할 수 있게 되었습니다.
    ],
  ),
)
