...
#projects(
  ...
  project_block(
    ...
    [
      = 개요
      사용자-사용자, 사용자-그룹 간 음성메시지를 담은 푸시알람을 설정한 스케줄에 따라 발송하는 기능을 제공하는 프로젝트 입니다.
      == 사용자 지정 스케줄에 따라 자동으로 발송되는 푸시 알림
      - 전달된 제목, 본문, 음성 파일을 FCM을 통해 사용자에게 발송될 수 있도록 했습니다.
      - 발송할 주기를 CRON표현식으로 푸시알람 테이블에 저장하고, 활성화된 푸시알람은 Quartz의 스케줄러에의해 관리되도록 했습니다.
      - 스케줄러에 등록된 푸시알람발송(Job)이 수행되면 발송할 대상을 확인해 푸시알람이 발송되도록 했습니다.
      == AWS S3
      - 사용자가 음성파일을 업로드시 보관및 재전송하기위해 버킷을 생성하여 저장하였습니다.
      == 깃허브 액션과 코드 디플로이를 통한 배포
      - 깃허브 액션과 코드디플로이를 활용해 배포 하여, 파이프라인을 개념과 구조를 익혔습니다.
      == 모바일 앱과 연동
      - 플러터를 활용해 앱을 만들고 푸시알람 수신시 백그라운드에서 함꼐 전달된 사용자 음성파일을 재생할 수 있도록 구현했습니다.
      == 수신자 목록 처리 및 Job 관리 최적화
      - 그룹단위 푸시알람 발송시 각각의 그룹원을 대상으로 하는 개별의 푸시알림발송Job을 스케줄러에 등록해 발송되도록 했음. 그룹원 변동시 스케줄러에 등록된 모든 변동된 그룹원에 대한 반영이 필요한 문제, 한눈에 들어오지않는 문제.-> 발송시 그룹, 개인발송의 타입을 추가해서 스케줄러에 등록, 발송시 해당 유저에게 발송. 결과적으로 스케줄러 관리가 편해짐  
    ],
  ),
)
